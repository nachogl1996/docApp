<div class="row text-center title">
	<h3 class="white">Mi consulta</h3>
	<p class="light-white">(haga click en un nombre para ver
		detalles)</p>
</div>
<div class="col-md-2">
	<h4>
		<a href="CargaHorarioMedicoServlet?inicio=${fechainicio }&mover=retrocede" class="btnamplia btn-white-fill"> < </a>
	</h4>
</div>
...
<div class="col-md-2 col-md-offset-2">
	<h4>
		<a href="CargaHorarioMedicoServlet?&inicio=${fechainicio }&mover=avanza" class="btnamplia btn-white-fill"> > </a>
	</h4>
</div>
<div class="row section-padded">
	...
	<c:forEach items="${horario}" var="cita">
		...
		<c:if test="${cita.status==0 }">
			<a class="btnreducido btn-green-fill" href="MedicoPedirCitaServlet?cita=${cita.date }">Asignar </a>
		</c:if>
		<c:if test="${cita.status == 1 }">
			...
			<c:if test="${cita.paciente != null }">
				<a class="btnreducido btn-yellow-fill" href="MedicoVerCitaServlet?cita=${cita.id }&abrir=si">P: ${cita.paciente.name } </a>
			</c:if>
		</c:if>
		<c:if test="${cita.status == 2 }">
			<a class="btnreducido btn-blue-fill" href="MedicoVerCitaServlet?cita=${cita.id }&abrir=si">P: ${cita.paciente.name } </a>
		</c:if>
		<c:if test="${cita.status == 3 }">
			<a class="btnreducido btn-orange-fill" href="MedicoVerCitaServlet?cita=${cita.id }&abrir=si">P:${cita.paciente.name } </a>
		</c:if>
		<c:if test="${cita.status == 4 }">
			<a class="btnreducido btn-gray-fill" href="MedicoVerCitaServlet?cita=${cita.id }&abrir=si">P:${cita.paciente.name } </a>
		</c:if>
		<%if (cuenta1==19){%>
			</div>
		<%}%>
