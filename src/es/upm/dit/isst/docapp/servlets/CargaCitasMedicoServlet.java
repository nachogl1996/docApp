...
@WebServlet("/CargaCitasMedicoServlet")
public class CargaCitasMedicoServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) 
		...
		if(medicops == null) {
			medico = (Medico) req.getSession().getAttribute("medicoesp");
		} else {
		...
		req.getSession().setAttribute("mesesp", FechasyCitasHandler.getInstance().mes(fechainicio));
		req.getSession().setAttribute("imprimefechasesp", imprimirfechas);
		req.getSession().setAttribute("fechainicioesp", formato.format(fechainicio));
		req.getSession().setAttribute("horarioesp", horariocitas);
		req.getSession().setAttribute("msge", medico.getEspecialidad().getName());
		req.getSession().setAttribute("msgm", medico.getName());
		req.getSession().setAttribute("medicoesp", medico);
